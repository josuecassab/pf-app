...
export default function TxnModalSelectBank() {
  ...
  const labels = useMemo(() => {
    ...
    return [TXN_FILTER_NULL_OPTION, ...banks];
  }, [banksData]);
  ...
  const onAccept = async () => {
    ...
    try {
      const res = await fetch(
        `${API_URL}/update_txn_bank/?table=${table}`,
        {
          method: "PUT",
      ...
      Alert.alert(
        "Error",
        error?.message ?? "No se pudo actualizar el banco.",
      );
    } finally {
      setLoading(false);
    }
