...
const HEADER = "Subcategoria";
const DROP_MAX_H = 480;

export default function TxnModalFilterSubcategory() {
  const { theme } = useTheme();
  const params = useLocalSearchParams();
  const { closeModal, submit } = useTxnFilterModalSubmit();

  const filterSubcategory = useMemo(
    () => parseJsonParam(params.filterSubcategoryJson, null),
    [params.filterSubcategoryJson],
  );

  const { data: subcategoriesData, isPending } = useSubcategories();

  const data = useMemo(() => {
    const subs = Array.isArray(subcategoriesData) ? subcategoriesData : [];
    return [TXN_FILTER_NULL_OPTION, ...subs];
  }, [subcategoriesData]);
